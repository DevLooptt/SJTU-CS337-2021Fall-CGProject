...
//edit
vector<XMFLOAT3> obj_vertex;
short hit[1280][720] = { 0 };
...
void RayTracer::UpdateFrame(uint8_t frameIndex, CXMVECTOR eyePt, CXMMATRIX viewProj, float timeStep)
{
	...
	if (!flag) {
		...
		char type;
		float a, b, c;
		ifstream in("Assets/output_1.obj");
		string line;
		while (getline(in, line)) {
			...
			if (type == 'v') {
				str >> a >> b >> c;
				//save vertexs
				obj_vertex.push_back(XMFLOAT3(a, b, -c));
			}
		}
		in.close();
	}
	...
	XMFLOAT3 result_tmp;
	float x, y;
	for (iter = obj_vertex.begin(); iter != obj_vertex.end(); ++iter) {
		vec_tmp = XMLoadFloat3(&*iter);
		XMStoreFloat3(&result_tmp, XMVector3TransformCoord(vec_tmp, viewProj));
