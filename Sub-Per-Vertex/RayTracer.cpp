...
// Zhichen Dong and Ruitao Zhu add an array that 
// stores the mapping relationship between vertices and pixels 
//--------------------------------------------------------------------------------------
#include<iostream>
#include<fstream>
#include<sstream>
	...
	uint8_t maxGBufferMips)
{
	...
	// An array that stores the mapping relationship between vertices and pixels
	static uint32_t testarray[1260* 720]={0};
	// view matrix
	static const float FOVAngleY = 0.785398163f;
	static const float zNear = 1.0f;
	static const float zFar = 1000.0f;
	static const float aspectRatio = 1280.0/760.0;
	const auto focusPt = XMVectorSet(-1.0f, 7.0f, 0.0f,0.0f);
	const auto eyePt = XMVectorSet(8.0f, 12.0f, -15.0f,0.0f);
	const auto view = XMMatrixLookAtLH(eyePt, focusPt, XMVectorSet(0.0f, 1.0f, 0.0f, 1.0f));
	const auto proj = XMMatrixPerspectiveFovLH(FOVAngleY, aspectRatio, zNear, zFar);
	...
	while (getline(in, line)) {
		...
		if (type == 'v') {
			str >> a >> b >> c;
			auto pos=XMFLOAT3(a, b, -c);
			XMVECTOR vec=XMLoadFloat3(&pos);
			XMStoreFloat3(&pos, XMVector3TransformCoord(vec, viewProj));

			x = (pos.x + 1) / 2;
			y = (pos.y + 1) / 2;

			int pixel_x = round(x * 1280);
			int pixel_y = 720 - round(y * 720);

			if (pixel_x >= 0 && pixel_y >= 0 && pixel_x < 1280 && pixel_y < 720)
				++testarray[pixel_x*720+pixel_y];
		}
	}
	in.close();
	// init sub-vertex-Buffer
	N_RETURN(createTB(pCommandList, 1260 * 720, testarray, uploaders), false);
...
bool RayTracer::createTB(XUSG::RayTracing::CommandList* pCommandList, uint32_t numPixel,
	const uint32_t* pData, vector<XUSG::Resource::uptr>& uploaders)
{
	auto& subvertexBuffers = m_subvertexBuffers[MODEL_OBJ];
	subvertexBuffers = IndexBuffer::MakeUnique();
	const uint32_t byteWidth = sizeof(uint32_t) * numPixel;
	N_RETURN(subvertexBuffers->Create(m_device.get(), byteWidth, Format::R32_UINT, ResourceFlag::NONE,
		MemoryType::DEFAULT, 1, nullptr, 1, nullptr, 1, nullptr, MemoryFlag::NONE, L"MeshTB"), false);
	uploaders.emplace_back(Resource::MakeUnique());

	return subvertexBuffers->Upload(pCommandList, uploaders.back().get(), pData,
		byteWidth, 0, ResourceState::NON_PIXEL_SHADER_RESOURCE);
}
...
bool RayTracer::createGroundMesh(RayTracing::CommandList* pCommandList, vector<Resource::uptr>& uploaders)
{
	...
	//Subvertex Buffer
	{
		...
		auto& testBuffers = m_subvertexBuffers[GROUND];
		testBuffers = IndexBuffer::MakeUnique();
		N_RETURN(testBuffers->Create(m_device.get(), sizeof(indices), Format::R32_UINT, ResourceFlag::NONE,
			MemoryType::DEFAULT, 1, nullptr, 1, nullptr, 1, nullptr, MemoryFlag::NONE, L"GroundTB"), false);
		uploaders.push_back(Resource::MakeUnique());

		N_RETURN(testBuffers->Upload(pCommandList, uploaders.back().get(), indices,
			sizeof(indices), 0, ResourceState::NON_PIXEL_SHADER_RESOURCE), false);
	}
	return true;
}
...
bool RayTracer::createPipelineLayouts()
{
	...
	// This is a pipeline layout that is shared across all raytracing shaders invoked during a DispatchRays() call.
	{
		...
		// Resource Binding: Subvertex Buffer
		pipelineLayout->SetRange(8U, DescriptorType::SRV, NUM_MESH, 0, 3);
...
bool RayTracer::createDescriptorTables()
{
	...
	// Subvertex buffer SRVs
	{
		Descriptor descriptors[NUM_MESH];
		for (auto i = 0u; i < NUM_MESH; ++i) descriptors[i] = m_subvertexBuffers[i]->GetSRV();
		const auto descriptorTable = Util::DescriptorTable::MakeUnique();
		descriptorTable->SetDescriptors(0, static_cast<uint32_t>(size(descriptors)), descriptors);
		X_RETURN(m_srvTables[SRV_TABLE_TB], descriptorTable->GetCbvSrvUavTable(m_descriptorTableCache.get()), false);
	}
...
void RayTracer::rayTrace(const RayTracing::CommandList* pCommandList, uint8_t frameIndex)
{
	...
	// Subvertex Buffer Descriptor Table
	pCommandList->SetComputeDescriptorTable(8U, m_srvTables[SRV_TABLE_TB]);
