...
// and make this shader support sub-pervertex ray tracing
//--------------------------------------------------------------------------------------
...
// Subvertex buffers
Buffer<uint>				g_subvertexBuffers[]		: register (t0, space3);
...
void raygenMain()
{
	...
	if (g_subvertexBuffers[1U][index.x*720+index.y] == 0)
		return;
	...
	// Calculate the radiation range of the current pixel according to g_subvertexBuffers
	uint2 index2;
	const int radius = 1 + 2 / g_subvertexBuffers[1U][index.x * 720 + index.y];

	// Render pixels in the circle with current payload.color
	for (int i = 0; i < radius; ++i) {
		for (int j = 0; j < radius; ++j) {
			if (i * i + j * j > radius* radius)
				return;
			index2 = uint2(DispatchRaysIndex().x - i, DispatchRaysIndex().y + j);
