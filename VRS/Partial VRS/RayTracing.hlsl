...
// Zhichen Dong and Ruitao Zhu modified Raygen shader with partial 2*2 VRS 
//--------------------------------------------------------------------------------------
...
bool getPrimarySurface(uint2 index, uint2 dim, out float3 N, out float3 V, out float3 P, out float4 color)
{
	...
	if (norm.w > 0.0)
	{
		...
		const float4 world = mul(float4(screenPos, 0, 1.0), l_rayGen.ProjToWorld);
...
void raygenMain()
{
	...
	//partial 2*2 VRS
	if ((index.x % 2 != 0 || index.y % 2 != 0)&&((index.x<150|| index.x>=850)||index.y>=600))
		return;
	...
	// Calculate the surrounding 2*2 pixels index
	const uint2 index2 = uint2(DispatchRaysIndex().x+1, DispatchRaysIndex().y);
	const uint2 index3 = uint2(DispatchRaysIndex().x , DispatchRaysIndex().y+1);
	const uint2 index4 = uint2(DispatchRaysIndex().x + 1, DispatchRaysIndex().y+1);
	...
	if ((index.x < 150 || index.x >= 850) || index.y >= 600) {
		// render the surrounding 2*2 pixels with current payload.Color
		g_renderTargets[HIT_GROUP_REFLECTION][index2] = payload.Color;
		g_renderTargets[HIT_GROUP_REFLECTION][index3] = payload.Color;
		g_renderTargets[HIT_GROUP_REFLECTION][index4] = payload.Color;
	}
